...
// ============================================================================
// TELA DE CONEXÃO (substitui o flash da sala vazia)
// ============================================================================
function TelaConectando({ payload, onCancelar }) {
  return (
    <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center gap-6 text-white z-50">
      <div className="w-20 h-20 rounded-full bg-slate-800 flex items-center justify-center mb-2 ring-2 ring-slate-700">
        {payload.avatar
          ? <img src={payload.avatar} alt={payload.nome} className="w-full h-full rounded-full object-cover" />
          : <span className="text-3xl font-bold text-slate-400">{payload.nome?.[0]?.toUpperCase()}</span>
        }
      </div>
      ...
      <button
        onClick={onCancelar}
        className="mt-4 px-6 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm transition-all"
      >
        Cancelar
      </button>
    </div>
...
// ============================================================================
// TELA DE ERRO DE ACESSO
// ============================================================================
function TelaErroAcesso({ mensagem, onVoltar }) {
  const msg = mensagem === 'Sala não existe.'
    ? 'A chave está incorreta ou esta sala já foi finalizada.'
    : mensagem;

  return (
    <div className="fixed inset-0 bg-slate-950 flex items-center justify-center z-50">
      <div className="flex flex-col items-center gap-4 bg-slate-900 border border-red-500/30 rounded-3xl px-12 py-10 shadow-[0_0_60px_rgba(239,68,68,0.1)] text-center max-w-sm w-full mx-6">
        ...
        <button
          onClick={onVoltar}
          className="mt-4 w-full py-3 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-xl transition-all active:scale-95 border border-slate-700"
        >
          Voltar ao painel
        </button>
      </div>
...
// AVATAR FALLBACK
// ============================================================================
function AvatarFallback({ nome, avatar }) {
  return (
    <div className="w-full h-full flex items-center justify-center bg-slate-900">
      {avatar
        ? <img src={avatar} alt={nome} className="w-20 h-20 rounded-full object-cover opacity-60 ring-2 ring-slate-700" />
        : <div className="w-20 h-20 rounded-full bg-slate-700 flex items-center justify-center text-2xl font-bold text-slate-400">
            {nome?.[0]?.toUpperCase() ?? '?'}
          </div>
      }
    </div>
          ...
          <div key={label} className={`flex-1 bg-slate-900 border border-${color}-500/30 hover:border-${color}-500/50 p-6 rounded-3xl shadow-xl flex flex-col transition-all`}>
            ...
            <div className="relative mb-6 flex-1">
              ...
              <textarea
                ...
                className={`w-full h-56 bg-slate-950 text-emerald-400 font-mono text-sm p-4 rounded-xl border border-slate-800 focus:border-${color}-500 outline-none resize-none shadow-inner`}
                spellCheck="false"
              />
            </div>
            <button
...
function SalaDeVideo({ payload, onSair, salaId, setSalaId }) {
  ...
  useEffect(() => {
    ...
    if (remainingMs <= 60000 && remainingMs > 0) setAvisoFim(true);
  ...
  useEffect(() => {
    ...
    connect({ ...payload, chave: payload.chave || salaId })
      ...
      .catch(() => {}); // erro tratado via status === 'error'
  }, []);

  // Aguardando conexão — mostra tela de loading
  if (status === 'connecting' || status === 'idle') {
    return <TelaConectando payload={payload} onCancelar={() => { leaveRoom(); onSair(); }} />;
  }

  // Erro de acesso — mostra tela de erro sem flash da sala
  if (status === 'error') {
    return <TelaErroAcesso mensagem={error} onVoltar={() => { leaveRoom(); onSair(); }} />;
  }

  const isProfessor = payload.papel === 'professor';
  const statusLabel = status === 'reconnecting' ? 'Reconectando...' : null;
  ...
  return (
    <div className="relative w-full h-screen bg-slate-950 overflow-hidden text-white">
      ...
      {/* Modal de encerramento */}
      {encerrando && <ModalEncerramento onFim={() => { leaveRoom(); onSair(); }} />}

      {/* Banner de aviso — último minuto */}
      {avisoFim && !encerrando && (
      ...
      <div className="absolute top-6 left-6 z-30 flex items-center gap-3 bg-slate-900/80 backdrop-blur-md px-6 py-3 rounded-2xl border border-white/10 shadow-lg">
        {payload.avatar && (
          <img src={payload.avatar} alt={payload.nome} className="w-6 h-6 rounded-full object-cover ring-1 ring-white/20" />
        )}
      ...
      {/* Vídeo remoto — auréola quando o outro fala */}
      ...
      {/* PiP arrastável — borda verde quando você fala */}
      <Draggable nodeRef={janelaVideoRef} bounds="parent" defaultPosition={{ x: 0, y: 0 }}>
        <div
          ref={janelaVideoRef}
          className={`absolute top-6 right-6 z-20 w-72 aspect-video bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border-2 transition-all duration-150 group cursor-grab active:cursor-grabbing
        ...
        >
          ...
          {isCamOn
            ...
            : <AvatarFallback nome={payload.nome} avatar={payload.avatar} />
          }
